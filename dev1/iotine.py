...
password = jwt.encode({'some': 'payload', "iat":datetime.datetime.utcnow(), "exp": datetime.datetime.utcnow() + datetime.datetime.timedelta(seconds=5)}, publicKey(), algorithm=JWTalgorithm)   
...
def publish(feed, val, callback=None):
    if pubstop == False:
        if callback == '':
            client.publish(str(jwt.encode({'topic': CONNSTRING+"/"+feed+"/"+USER, 'value': str(val), "iat":datetime.datetime.utcnow(), "exp": datetime.datetime.utcnow() + datetime.datetime.timedelta(seconds=5)}, publicKey(), algorithm=JWTalgorithm)), '')
        else:    
            client.publish(str(jwt.encode({'topic': CONNSTRING+"/"+feed+"/"+USER, 'value': str(val), "iat":datetime.datetime.utcnow(), "exp": datetime.datetime.utcnow() + datetime.datetime.timedelta(seconds=5)}, publicKey(), algorithm=JWTalgorithm)), '', callback=callback)
