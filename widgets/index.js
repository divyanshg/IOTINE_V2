...
var updateWidget = (user, app, widget) => {
    return new Promise((resolve, reject) => {
        con.query('UPDATE widgets SET name = ?, feed = ?, backgroundColor = ?, borderColor = ?, borderWidth = ?, device = ?, label = ? WHERE id = ? ', [widget.name, widget.feed, widget.datasets[0].backgroundColor, widget.datasets[0].borderColor, widget.datasets[0].borderWidth, widget.config.device, widget.datasets[0].label, widget.id], (err, res) => {
            if(err) return reject(err);
            resolve(res)
        })
    })
